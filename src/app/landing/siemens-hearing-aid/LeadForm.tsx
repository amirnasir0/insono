...
export default function LeadForm({ compact = false }: { compact?: boolean }) {
  const { gclid, utm_source, utm_medium, utm_campaign, utm_term, utm_content } = useUtmParams({
    utm_campaign: "siemens-hearing-aid",
  });
  ...
  return (
    ...
    >
      ...
      <input type="hidden" name="utm_campaign" value={utm_campaign || "siemens-hearing-aid"} />
      ...
      >
        {compact ? "📞 Get Free Expert Callback" : "Download Siemens Price List"}
      </button>

      <p className="text-xs text-gray-400 text-center">
        You&apos;ll instantly receive a full price list on WhatsApp to compare all
        Siemens &amp; Signia models — 100% free, no obligation.
      </p>
    </form>
  );
}
